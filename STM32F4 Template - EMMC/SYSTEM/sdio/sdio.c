...
//	RCC_APB2PeriphResetCmd(RCC_APB2Periph_SDIO, ENABLE);//SDIO????
  ...
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource8,  GPIO_AF_SDIO);//SDIO0
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource9,  GPIO_AF_SDIO);//SDIO1
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource10, GPIO_AF_SDIO);//SDIO2
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource11, GPIO_AF_SDIO);//SDIO3
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource8,  GPIO_AF_SDIO);//SDIO4
	GPIO_PinAFConfig(GPIOB, GPIO_PinSource9,  GPIO_AF_SDIO);//SDIO5
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource6,  GPIO_AF_SDIO);//SDIO6
	GPIO_PinAFConfig(GPIOC, GPIO_PinSource7,  GPIO_AF_SDIO);//SDIO7
	
  GPIO_PinAFConfig(GPIOC, GPIO_PinSource12, GPIO_AF_SDIO);//SDIO_CK
  GPIO_PinAFConfig(GPIOD, GPIO_PinSource2,  GPIO_AF_SDIO);//SDIO)CMD
...
EmmcError EmmcInitializeCards(EmmcCardInfo *E)
{
  ...
  if (SDIO_GetPowerState() == SDIO_PowerState_OFF)
  {
    ...
    SDIO_CmdInitStructure.SDIO_Argument = 0x00010000;
	...
	Result = EmmcSelectDeselect(0x00010000);
	if (EMMC_OK != Result)
	{
		return(Result);
	}
...
EmmcError EmmcBusTest(void)
{
    ...
    SDIO_CmdInitStructure.SDIO_CmdIndex = EMMC_CMD_HS_BUSTEST_WRITE;
    SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
    SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
    SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
    SDIO_SendCommand(&SDIO_CmdInitStructure);
	  errorstatus = CmdResp1Error(EMMC_CMD_HS_BUSTEST_WRITE);
...
EmmcError EmmcGetCardInfo(EmmcCardInfo *E, u32 *CSD_Tab, u32 *CID_Tab, u16 Rca)
{
	  ...
	  tmp = (u8)((CSD_Tab[0] & 0xFF000000) >> 24);      
		...
		E->CardCapacity = (u32)((u32)(E->EmmcExtCsd.EXT_CSD.SEC_COUNT[3] << 24 | E->EmmcExtCsd.EXT_CSD.SEC_COUNT[2] << 16 | 
    ...
    /**************************CSD REG*****************************/
		...
		E->EmmcCid.Reserved1 = (tmp & 0xFC)>>2;
		E->EmmcCid.CardorBGA = (tmp & 0x03);
...
EmmcError EmmcReadExtCsd(EmmcCardInfo *E)
{
	...
	u32 count = 0;
  u32 *ExtCsdBuf;
	ExtCsdBuf = (u32 *)(&(E->EmmcExtCsd.CsdBuf[0]));
...
//    SDIO_DMACmd(ENABLE);
//    EmmcLowLevelDMARxConfig((u32 *)readbuff, BlockSize);
//	
...
//  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
//  SDIO_DMACmd(ENABLE);
//  EmmcLowLevelDMARxConfig((u32 *)readbuff, (NumberOfBlocks * BlockSize));
//	  OS_EXIT_CRITICAL();
//	errorstatus = EmmcWaitReadOperation();
//	if(errorstatus !=  EMMC_OK)
//	{
...
//	
//  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
//  EmmcLowLevelDMATxConfig((u32 *)writebuff, BlockSize);
//  SDIO_DMACmd(ENABLE);
...
//  SDIO_ITConfig(SDIO_IT_DCRCFAIL | SDIO_IT_DTIMEOUT | SDIO_IT_DATAEND | SDIO_IT_RXOVERR | SDIO_IT_STBITERR, ENABLE);
//  SDIO_DMACmd(ENABLE);
//	EmmcLowLevelDMATxConfig((u32 *)writebuff, (NumberOfBlocks * BlockSize));
//  OS_EXIT_CRITICAL();
//	
...
EmmcError EmmcStopTransfer(void)
{
  ...
  /*!< Send CMD12 STOP_TRANSMISSION  */
  SDIO_CmdInitStructure.SDIO_Argument = (u32)0;//(MyEmmcCardInfo.RCA << 16);
  SDIO_CmdInitStructure.SDIO_CmdIndex = EMMC_CMD_STOP_TRANSMISSION;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
...
//				delay_ms(100);
			...
			/* CMD6 改成8bit数据宽度*/ 
			delay_ms(10);
			SDIO_CmdInitStructure.SDIO_Argument = EMMC_8BIT_REG;
			SDIO_CmdInitStructure.SDIO_CmdIndex = EMMC_CMD_HS_SWITCH;
			SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
			SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
			SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
...
EmmcError IsCardProgramming(u8 *pstatus)
{
  ...
  SDIO_CmdInitStructure.SDIO_Argument = (u32) MyEmmcCardInfo.RCA << 16;
  SDIO_CmdInitStructure.SDIO_CmdIndex = EMMC_CMD_SEND_STATUS;
  SDIO_CmdInitStructure.SDIO_Response = SDIO_Response_Short;
  SDIO_CmdInitStructure.SDIO_Wait = SDIO_Wait_No;
  SDIO_CmdInitStructure.SDIO_CPSM = SDIO_CPSM_Enable;
